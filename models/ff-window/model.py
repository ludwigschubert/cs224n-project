...
if args.runmode == "test":
    predictions_dir_name = "_".join(["ff-window", LOGDIR, args.trained_on, args.dataset_name])
...
input_concat = tf.concat(input_embed, 0)

W1 = tf.get_variable("W1", shape=[INPUT_MAX*GLV_DIM, HIDDEN_STATE_SIZE], initializer=tf.contrib.layers.xavier_initializer())
b1 = tf.Variable(tf.constant(0.0, shape=[HIDDEN_STATE_SIZE]))
h = tf.nn.tanh(tf.matmul(input_concat, W1) + b1)
